...
public class SorterTabell {
    ...
    flettesortering(T[] a, int first, int last) {
    ...
    flettesortering(T[] a, T[] tempTab, int forste, int siste) {
        ...
        Dette er et triks for å vise at vi har tenkt på basistilfelle
        Kunne i stedenfor utelatt dette og testet om første < siste og så gått rett på delen i else
         */
    ...
    private static final int MIN_GRENSE = 20;
    ...
    public static <T extends Comparable<? super T>> void sorterVedInnsettingS(T[] a, int forste, int siste) {
    ...
    public static <T extends Comparable<? super T>> void sorterVedInnsetting(T[] a, int forste, int siste) {
        ...
        for (int i = forste + 1; i <= siste; i++) {
            ...
            /*
            Denne versjonen (swap): 3 bytter per tilordning som er mer enn den andre (switch)
            Begge er O(n^2), men shift har færre tilordninger
             */
            while (j > forste && a[j].compareTo(a[j-1]) < 0) {
                T temp = a[j];
                a[j] = a[j - 1];
                a[j - 1] = temp;
                j--;
    ...
    public static <T extends Comparable<? super T>> void sorterVedInnsettingToOgTo(T[] a) {
        ...
        for (int i = 0; i < n - 1; i += 2) {
            ...
            // Flytt to plasser så lenge største er mindre
        ...
        // Hvis oddetall av elementer: sett inn siste på vanlig måte
        if (n % 2 != 0) {
            T temp = a[n - 1];
            int j = n - 2;
